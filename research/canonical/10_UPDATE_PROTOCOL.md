...
**Current application (D039):** after D037 parked C011, `20_C011_EXECUTION_PROTOCOL.md` is retired from active Project Sources but remains byte-identically preserved in the C011 Git/reproducibility archive. The Decision Ledger, Candidate Registry and journey record retain its governing history. This keeps the replacement v0.28.0 source set within 25 files while adding the authoritative C012 prosecution, C013 prosecution and C013 controlled-execution protocol.
...
- Recorded the D039 source-cap retirement of the parked C011 protocol from active Project Sources while preserving it in the durable C011 archive.
- Fixed the v0.28.0 replacement-set maximum at 25 files including `MANIFEST.json`.
