...
for a in CFG['anchors']:
    did=a['dataset_id']
    meta=call(f'https://data.cdc.gov/api/views/{did}')
    count=call(f'https://data.cdc.gov/resource/{did}.json?%24select=count%28%2A%29')
    sample=call(f'https://data.cdc.gov/resource/{did}.json?%24limit=1')
    fields=[]
    title=None
    if meta['ok']:
        title=meta['json'].get('name')
        fields=[c.get('fieldName') for c in meta['json'].get('columns',[]) if c.get('fieldName')]
    row_count=None
